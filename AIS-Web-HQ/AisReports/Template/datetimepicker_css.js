	...
	if (exDateTime!="")	{ //Parse existing Date String
		...
		if ((Cal.Format.toUpperCase()=="DDMMYYYY") || (Cal.Format.toUpperCase()=="DDMMMYYYY")) {
			if (DateSeparator=="") {
				...
				strYear=String(parseInt(strYear,10))
			}
			else {
...
function RenderCssCal(bNewCal) {
	...
	if (Cal.Scroller=="DROPDOWN") {
		...
		for (i = StartYear,j=StartYear; i <= (dtToday.getFullYear() + EndYear);i++,j++)	{
			if (i==Cal.Year)
				SelectStr="Selected";
			else
				SelectStr="";
  ...
  //******************End Month and Year selector in arrow******************************
	...
	if ((ShowMonthYear)&&(Cal.Scroller=="DROPDOWN")) {
		vCalHeader+="<tr><td colspan='7' class='calR'>\n<font color='"+MonthYearColor+"'>"+Cal.GetMonthName(ShowLongMonth)+" "+(parseInt(Cal.Year,10))+"</font>\n</td></tr>\n";
	    calHeight += 19;
	}
...
function FormatDate(pDate)
{
	...
	if (this.Format.toUpperCase()=="DDMMYYYY")
		return (pDate+DateSeparator+MonthDigit+DateSeparator+String(parseInt(this.Year,10)) );
...
function dateAdd(p_Interval, p_Number, p_Date) {
    ...
    var dt = new Date(dDate[2], dDate[1] - 1, dDate[0]);
    dt = newDayAdd(dt, -1);
    ...
    return retDate + "/" + retMonth + "/" + dt.getFullYear();
}
