                        ...
                        For Each dr As DataRow In dt.Rows
                            If dr("Active").ToString = "Y" Then
                                If File.Exists(dr("FileName")) = True Then
                                    ...
                                    Dim AlarmCode As String = "ALM_DATAFILESIZE_" 'conf.ServerName & "_DATAFILESIZE_" & ff.FullName & "_" & ff.Directory.Root.Name
                                    Dim CurrFileSize As Double = Convert.ToDouble(ff.Length / (1024 ^ 3))
                                    If CurrFileSize > Convert.ToDouble(dr("FileSizeCritical")) Then
                                        ProcessFileSizeAlarm(awDt, conf, "CRITICAL", eng, CurrFileSize, conf.AlarmMethod, Convert.ToDouble(dr("FileSizeCritical")), AlarmCode & "CRITICAL", ff)
                                    ElseIf CurrFileSize > Convert.ToDouble(dr("FileSizeMajor")) Then
                                        ProcessFileSizeAlarm(awDt, conf, "MAJOR", eng, CurrFileSize, conf.AlarmMethod, Convert.ToDouble(dr("FileSizeMajor")), AlarmCode & "MAJOR", ff)
                                    ElseIf CurrFileSize > Convert.ToDouble(dr("FileSizeMinor")) Then
                                        ProcessFileSizeAlarm(awDt, conf, "MINOR", eng, CurrFileSize, conf.AlarmMethod, Convert.ToDouble(dr("FileSizeMinor")), AlarmCode & "MINOR", ff)
                                    ElseIf CurrFileSize <= Convert.ToDouble(dr("FileSizeMinor")) Then
                                        If awDt.Rows.Count > 0 Then
